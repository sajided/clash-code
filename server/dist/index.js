...
import { createRoom, addPlayerToRoom, getRoomByCode, getRoomBySocket, getPlayerInfos, getPlayerNames, playSabotageCard, resetRoundMana, startManaTicker, } from "./game.js";
...
io.on("connection", (socket) => {
    ...
    socket.on("player_ready", () => {
        ...
        if (!allReady)
            return;
        room.status = "playing";
        room.problems = getProblemsForRounds(room.roundRatings);
        room.round = 1;
        resetRoundMana(room);
        const problem = room.problems[0];
        if (!problem)
            return;
        const ticker = startManaTicker(room, (sid, mana) => {
            io.to(sid).emit("mana_update", { mana });
        });
        manaTickers.set(room.id, ticker);
    ...
    socket.on("play_card", (payload) => {
        ...
        if (result.applied) {
            io.to(targetId).emit("apply_effect", {
                type: card,
                duration: card === "blind" ? 5 : card === "freeze" ? 3 : undefined,
            });
        }
        else {
    ...
    socket.on("submit_code", async (_code, language) => {
        ...
        if (accepted) {
            ...
            for (const [sid, p] of room.players)
                scores[sid] = p.score;
            if (player.score >= 2) {
                clearInterval(manaTickers.get(room.id));
                manaTickers.delete(room.id);
