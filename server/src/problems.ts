...
import { supabase, type ProblemRow } from "./db.js";
import { getProblemByDifficultyFromMerged } from "./merged-problems.js";

export const HARDCODED_PROBLEMS: Problem[] = [
  {
    id: "cf-4a",
    title: "Watermelon",
...
function ratingToDifficulty(rating: number): ProblemDifficulty {
  if (rating <= 1000) return "Easy";
  if (rating <= 1400) return "Medium";
  return "Hard";
}

function normalizeRow(row: ProblemRow): Problem {
...
async function getProblemByDifficultyFromDb(difficulty: ProblemDifficulty): Promise<Problem | null> {
  ...
  const row = data[Math.floor(Math.random() * data.length)] as ProblemRow;
  return normalizeRow(row);
}

function pickByDifficulty(problems: Problem[], difficulty: ProblemDifficulty, excludeIds: Set<string>): Problem {
  const filtered = problems.filter(
    (p) => !excludeIds.has(p.id) && (p.difficulty ?? ratingToDifficulty(p.rating ?? 800)) === difficulty
  );
...
export async function getProblemByDifficulty(difficulty: ProblemDifficulty): Promise<Problem> {
...
export async function getProblemsForRounds(
  selectedDifficulty?: ProblemDifficulty
): Promise<[Problem, Problem, Problem]> {
  const diff: ProblemDifficulty = selectedDifficulty ?? "Medium";
