...
const lastManaGrantAt = new Map<string, number>();
...
io.on("connection", (socket) => {
  ...
  socket.on("set_config", (payload) => {
    ...
    if (payload.roundRatings) room.roundRatings = payload.roundRatings;
    if (payload.problemDifficulty) room.problemDifficulty = payload.problemDifficulty;
  });

  socket.on("set_language", (language) => {
    const room = getRoomBySocket(rooms, socket.id);
    if (!room || room.status !== "waiting" && room.status !== "configuring") return;
  ...
  socket.on("player_ready", async () => {
    const room = getRoomBySocket(rooms, socket.id);
    if (!room) return;
    if (room.status !== "waiting" && room.status !== "configuring") return;
  ...
  socket.on("get_game_state", (code: string) => {
    const room = getRoomByCode(rooms, code);
    if (!room || room.status !== "playing" || !room.problems) return;
  ...
  socket.on("submit_code", async (_code, language) => {
    ...
    if (accepted) {
      ...
      for (const [sid, p] of room.players) scores[sid] = p.score;
  ...
  socket.on("keystroke_chunk", (charCount: number) => {
    const room = getRoomBySocket(rooms, socket.id);
    if (!room || room.status !== "playing") return;
    const now = Date.now();
    const last = lastManaGrantAt.get(socket.id) ?? 0;
    if (now - last < MANA_GRANT_COOLDOWN_MS) return;
  ...
  socket.on("disconnect", () => {
    ...
    if (room) {
      ...
      if (room.players.size === 0) {
        ...
        if (newHost) room.hostSocketId = newHost;
